...
# ==========================================================
#  运行模式判断：
#  1) 环境变量模式（GitHub Actions / 服务器）—— 读取环境变量
#  2) 本地模式 —— 读取/生成 config.json
# ==========================================================
ENV_CLASSID = (os.environ.get("ClassID") or "").strip()
ENV_X = (os.environ.get("X") or "").strip()
ENV_Y = (os.environ.get("Y") or "").strip()
ENV_ACC = (os.environ.get("ACC") or "").strip()
ENV_SEARCHTIME = (os.environ.get("SearchTime") or "").strip()   # 预留，未使用
ENV_COOKIE = (os.environ.get("MyCookie") or "").strip()
ENV_TOKEN = (os.environ.get("token") or "").strip()

# 只要班级ID、经纬度、Cookie 都存在，就认为是环境变量模式
USE_ENV = bool(ENV_CLASSID and ENV_X and ENV_Y and ENV_COOKIE)
...
# ==========================================================
if USE_ENV:
    # ---------------- 环境变量模式（GitHub Actions） ----------------
    print("----------环境变量模式（GitHub Actions）----------")
    ClassID = ENV_CLASSID
    X = ENV_X
    Y = ENV_Y
    ACC = ENV_ACC if ENV_ACC else "0"
    pushtoken = ENV_TOKEN
    scheduletime = ""   # Actions 由 workflow_dispatch 手动触发，直接执行一次
    debug = (os.environ.get("DEBUG", "false").strip().lower() == "true")

    # Cookie 支持多用户：在 Secret 中用「换行」分隔多个 Cookie
    # 例如：
    #   username=小明;remember_student_xxxx=aaaa
    #   username=小红;remember_student_xxxx=bbbb
    _raw_cookie = ENV_COOKIE.replace("\\n", "\n")
    Cookies = [c.strip() for c in re.split(r"[\r\n]+", _raw_cookie) if c.strip()]
    ...
    with open(file_path, "r", encoding="utf-8") as file:
        ...
        if not json_data["configLock"]:
            ...
            # 3. 写回JSON文件
            with open(file_path, "w", encoding="utf-8") as file:
                json.dump(json_data, file, indent=4, ensure_ascii=False)  # indent 设置缩进为4个空格
            print("数据已保存到" + current_directory + "下的config.json中。")
        else:
            print("----------欢迎回来----------")
...
print("----------信息----------")
print("班级ID:" + str(ClassID))
print("纬度:" + str(X))
print("经度:" + str(Y))
print("海拔:" + str(ACC))
# print("检索间隔:" + str(SearchTime))
print("Cookie数量:" + str(len(Cookies)))
print("定时:" + str(scheduletime))
print("通知token:" + str(pushtoken))
if debug:
...
def thisTime(hour, minute):
    ...
    target_time_struct = time.strptime(
        today_date + " " + str(target_hour) + ":" + str(target_minute) + ":00",
        "%Y-%m-%d %H:%M:%S"
    )
...
def qiandao(theCookies):
    ...
    url = 'http://k8n.cn/student/course/' + str(ClassID) + '/punchs'
    errorCookie = []
    nullCookie = 0
    # 多用户检测签到
    for uid in range(0, len(theCookies)):
        ...
        if result:
            ...
            headers = {
                ...
                'Referer': 'http://k8n.cn/student/course/' + str(ClassID),
            ...
            if title_tag and "出错" not in title_tag.text:
                ...
                if matches:
                    for match in matches:
                        url1 = "http://k8n.cn/student/punchs/course/" + str(ClassID) + "/" + match
                        ...
                        printLog("info", "用户UID[%d%s] | 签到请求已发送： 签到ID[%s] 签到定位[%s,%s] 签到海拔[%s]" % (
                            uid + 1, username_string, match, newX, newY, ACC))
                        ...
                        if response.status_code == 200:
                            ...
                            if div_tag:
                                ...
                                if pushtoken != "" and h1_text == "签到成功":
                                    url = 'http://www.pushplus.plus/send?token=' + pushtoken + \
                                          '&title=' + "班级魔法自动签到任务" + '&content=' + h1_text
                                    requests.get(url)
                                continue  # 返回到查找进行中的签到循环
                            else:
                                print("未找到 <h1> 标签，可能存在错误")
                                printLog("info", "用户UID[%d%s] | 未找到 <h1> 标签，可能存在错误或签到成功" % (
                                    uid + 1, username_string))
                        else:
                            print("请求失败，状态码:", response.status_code)
                            printLog("error", "用户UID[%d%s] | 请求失败，状态码: %d" % (
                                uid + 1, username_string, response.status_code))
...
def job():
    ...
    if len(errorCookie) > 0:
        ...
        time.sleep(300)
        errorCookie, _ = qiandao(errorCookie)
        if len(errorCookie) > 0:
...
# ==========================================================
if USE_ENV:
    # ---------- GitHub Actions 模式：执行一次后退出 ----------
    print("☆环境变量模式：立即执行一次签到任务☆")
    job()
    print("★GitHub Actions 签到任务执行完毕★")
else:
    # ---------- 本地模式 ----------
    if scheduletime != "":
